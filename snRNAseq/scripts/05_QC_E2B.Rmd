...
treatment <- "E2B"
# Remove sample
metadata <- subset(metadata, pig_id == "E2")
metadata$sampleID <- factor(metadata$pig_id, levels = c(metadata$pig_id))
samples <- "E2B"
```
...
if (file.exists("../rObjects/annotation.rds")) {
  ...
  gtf.file <- paste0(pathToRef, "Sus_scrofa.Sscrofa11.1.107.filteredCellranger.gtf")
  genes <- rtracklayer::import(gtf.file)
  genes <- as.data.frame(genes)
  genes <- genes[genes$type == "gene",]
  saveRDS(genes, "../rObjects/annotation.rds")
}
write.table(genes, "../rObjects/genes.tsv", row.names = F, quote = F, sep = "\t")
gene_type_table <- table(genes$gene_biotype)
write.table(gene_type_table, "../rObjects/gene_type_table.tsv", row.names = F, quote = F, sep = "\t")
...
# Setup seurat object
```{r seurat_object}
prefix <- "../pipseeker_output/"
suffix <- "/SoupX_autoFraction/" # SoupX_autoFraction
...
if (file.exists(paste0("../rObjects/", treatment, ".rds"))) {
  ...
  for (i in treatment) {
    print(i)
    obj <- CreateSeuratObject(Read10X(paste0(prefix,i,suffix)))
    assign(i, obj)
  }
  # merge objects
  dataObject <- merge(x = E2B,
                 add.cell.ids = treatment, 
                 project = "E2B")
...
# save v2
v2
path <- paste0("../results/violin/",treatment,"_percent_genes_raw")
saveToPDF(paste0(path, ".pdf"), width = 8.5, height = 4)
dev.off()
...
dataObject.filtered <- RunPCA(object = dataObject.filtered)
```
...
DimHeatmap(dataObject.filtered, dims = 1:6, cells = 500, balanced = TRUE)
path <- paste0("../results/pca/",treatment,
               "_pca_genes")
...
```{r}
# 0.4
umap0.2 <- DimPlot(dataObject.filtered,
...
Idents(dataObject.filtered) <- dataObject.filtered$seurat_clusters
...
write.table(sample_ncells, 
            paste0("../results/nuclei_count/",
                   treatment, 
                   "_nuclei_per_cluster_sample.txt"),
            quote = FALSE, sep = "\t")
...
saveRDS(dataObject.filtered, paste0("../rObjects/",treatment,"_unannotated.rds"))
```
