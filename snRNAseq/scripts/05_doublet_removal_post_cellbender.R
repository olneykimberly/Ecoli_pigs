...
for (i in 1:length(dataObject.split)) {
  ...
  print(i)
  pigs_sample <- NormalizeData(dataObject.split[[i]])
  sampleID <- levels(droplevels(pigs_sample@meta.data$sample))
  pigs_sample <- FindVariableFeatures(pigs_sample, selection.method = "vst", nfeatures = 2000)
  pigs_sample <- ScaleData(pigs_sample)
  pigs_sample <- RunPCA(pigs_sample)
  
  # get significant PCs
  stdv <- pigs_sample[["pca"]]@stdev
  sum.stdv <- sum(pigs_sample[["pca"]]@stdev)
  ...
  # run umap
  pigs_sample <- RunUMAP(pigs_sample, dims = 1:min.pc, reduction = "pca")
  
  # cluster
  pigs_sample <- FindNeighbors(object = pigs_sample, dims = 1:min.pc)                           
  pigs_sample <- FindClusters(object = pigs_sample, resolution = 0.2)
  
  # Assign identity of clusters
  Idents(object = pigs_sample) <- "seurat_clusters"

  # number of cells in each cluster
  n_cells <- FetchData(pigs_sample, vars = c("ident")) %>% dplyr::count(ident) %>%tidyr::spread(ident, n)
  
  ## pK Identification (no ground-truth) 
  sweep.res.list <- paramSweep(pigs_sample, PCs = 1:min.pc, sct = FALSE)
  ...
  annotations <- pigs_sample@meta.data$individual_clusters
  homotypic.prop <- modelHomotypic(annotations) 
  nExp_poi <- round(pK_value*nrow(pigs_sample@meta.data))
  nExp_poi.adj <- round(nExp_poi*(1-homotypic.prop))
  # Run DoubletFinder with varying classification  
  pigs_sample <- doubletFinder(pigs_sample, PCs = 1:min.pc, 
                pN = 0.25, pK = pK_value, nExp = nExp_poi.adj, 
                reuse.pANN = FALSE, sct = FALSE)
  
  # set DF class for calling doublets
  DF_class <- pigs_sample@meta.data[, grep("DF.classifications",colnames(pigs_sample@meta.data)),]
  ...
  #only keep singlets
  pigs_sample_singlets <- subset(pigs_sample, subset = CellTypes_DF == "Singlet")
  

  # number of cells in each cluster per and post removing doublets
  n_cells_singlets <- FetchData(pigs_sample_singlets, vars = c("ident")) %>% dplyr::count(ident) %>% tidyr::spread(ident, n)
  n_cells_singlets
  ...
  print(b1)
  dev.off()
  f2 <- FeaturePlot(pigs_sample_singlets, 
            reduction = "umap", 
            features = c("nFeature_RNA", "nCount_RNA", 
                         "cell.complexity", "percent.mt"),
            pt.size = 0.4, 
  ...
  dataObject.split[[i]] <- pigs_sample_singlets
}
...
dataObject.singlets <- merge(x = dataObject.split[[1]],
                       ...
                       dataObject.split[[7]], dataObject.split[[8]]),
                       project = paste0("pigs"))
