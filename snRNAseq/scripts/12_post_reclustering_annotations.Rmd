...
projectID <- "pigs_cellbender_after_recluster_harm_int"
...
path <- paste0("snRNAseq/results/tree/",projectID,
               "_UMAP_unannotated")
...
## Cluster 15 - OPC/polydendrocytes
```{r cluster15}
...
## Cluster 17 - oligo - saline 1 sample only
```{r cluster17}
...
## Cluster 19 - oligo - saline 1 sample only
```{r cluster19}
...
## Cluster 20 - VLMC - Vascular leptomeningeal cells 
```{r cluster17}
...
## Cluster 23 - astrocyte/olig - NOISE!
```{r cluster23}
...
FeaturePlot(object = subset(dataObject, seurat_clusters == "23"),
              reduction = "umap",
              features = c("TYROBP", "MOG", "AQP4", "RBFOX3"))
```
...
FeaturePlot(object = subset(dataObject, seurat_clusters == "28"),
              reduction = "umap",
              features = c("TYROBP", "MOG", "AQP4", "RBFOX3"))
```
...
# Save RDS 
```{r save_annotations}
saveRDS(dataObject.annotated.clean, paste0("snRNAseq/rObjects/", projectID, "_noise_removed_after_annotation.rds"))
```
