...
# Genes 
```{r gene_info}
if (file.exists("snRNAseq/rObjects/annotation.rds")) {
  genes <- readRDS("snRNAseq/rObjects/annotation.rds")
} else {
  ...
  saveRDS(genes, "snRNAseq/rObjects/annotation.rds")
}
...
```{r QC_metrics}
nCount.min <- 500 
nCount.max <- 150000  # max is 245982 
nFeature.min <- 100
nFeature.max<- 12000
...
# Setup seurat object
```{r seurat_object}
prefix <- "snRNAseq/cellbender/"
suffix <- "_filtered_seurat.h5"

if (file.exists(paste0("snRNAseq/rObjects/", projectID, ".rds"))) {
