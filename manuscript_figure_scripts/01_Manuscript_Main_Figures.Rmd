...
```{r UMAP}
# a) UMAP
#dataObject$group <- factor(dataObject$group, levels = c("Saline", "Ecoli"))
...
UMAP_split <- dittoDimPlot(object = dataObject,
                         var = "individual_clusters",
                         size = .25,
                         split.by = "group",
                         reduction.use = "umap",
...
#     )
# Option1
# path <- paste0("snRNAseq/manuscript_figures/Figure_2_umap_rel_abun_dotplot")
...
saveToPDF(paste0(path, ".pdf"), width = 7, height = 5.25)
```
...
up_enrich_results <-
  read_excel(
    paste0(
      "snRNAseq/results/metascape/endothelial_up/metascape_result.xlsx"
    ), sheet = 2
  )
...
down_enrich_results <-
  read_excel(
    paste0(
      "snRNAseq/results/metascape/endothelial_down/metascape_result.xlsx"
    ), sheet = 2
  )
...
up_enrich_results <-
  read_excel(
    paste0(
      "snRNAseq/results/metascape/mural_up/metascape_result.xlsx"
    ), sheet = 2
  )
...
down_enrich_results <-
  read_excel(
    paste0(
      "snRNAseq/results/metascape/mural_down/metascape_result.xlsx"
    ), sheet = 2
  )
...
up_enrich_results <-
  read_excel(
    paste0(
      "snRNAseq/results/metascape/microglia_up/metascape_result.xlsx"
    ), sheet = 2
  )
...
down_enrich_results <-
  read_excel(
    paste0(
      "snRNAseq/results/metascape/microglia_down/metascape_result.xlsx"
    ), sheet = 2
  )
...
# Step 1: Create temporary image file
tmp_file <- tempfile(fileext = ".png")

# Step 2: Save the Ecoli plot to this file
png(tmp_file, width = 1000, height = 1000, res = 300)
compare_heatmap
dev.off()
# Step 3: Read the image and convert to ggplot-compatible object
compare_heatmap_plot <- as.ggplot(image_read(tmp_file))

gg1 <- compareInteractions(cellchat, show.legend = F, group = c(1,2), color.use = c("black", "green3"))+ theme(
  strip.text = element_text(size = 8),
  axis.text.y = element_text(size = 8),
  axis.text.x = element_text(size = 8),
  axis.title.x = element_text(size = 8),
  plot.margin = margin(0.2, 0.2, 0.2, 0.2, "cm")) +
  scale_y_discrete(expand = expansion(mult = c(0, 0.2))) # Adjusts y-axis expansion

gg2 <- compareInteractions(cellchat, show.legend = F, group = c(1,2), measure = "weight", color.use = c("black", "green3"), digits = 2)+ theme(
  strip.text = element_text(size = 8),
  axis.text.y = element_text(size = 8),
  axis.text.x = element_text(size = 8),
  axis.title.x = element_text(size = 8),
...
# Step 2: Save the Ecoli plot to this file
png(tmp_file, width = 1000, height = 1000, res = 350)
netVisual_chord_gene(object.list$Ecoli, sources.use = 2, targets.use = c(1:8),
                     lab.cex = 0.5, title.name = NULL, show.legend = FALSE)
dev.off()
# Step 3: Read the image and convert to ggplot-compatible object
Ecoli_plot <- as.ggplot(image_read(tmp_file))
Ecoli_plot
path <- paste0("manuscript_figures/Ecoli_plot")
saveToPDF(paste0(path, ".pdf"), width = 3.5, height = 3)
...
# Step 2: Save the Ecoli plot to this file
png(tmp_file, width = 1000, height = 1000, res = 350)
netVisual_chord_gene(object.list$Saline, sources.use = 2, targets.use = c(1:8),
                     lab.cex = 0.5, title.name = NULL, show.legend = FALSE)
dev.off()

# Step 3: Read the image and convert to ggplot-compatible object
Saline_plot <- as.ggplot(image_read(tmp_file))
Saline_plot
path <- paste0("manuscript_figures/Saline_plot")
...
figure6
path <- paste0("manuscript_figures/Figure_6_cellchat")
saveToPDF(paste0(path, ".pdf"), width = 7, height = 6)
```
