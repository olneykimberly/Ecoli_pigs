...
colnames(count_data) <- brain_metadata$counts_name
...
all.equal(colnames(count_data), (brain_metadata$counts_name)) # counts columns match the sample IDs
```
...
sample_group_color <- c("gray", "darkgreen")[dge$samples$condition]
...
```{r varpart}
form_varPart <- ~ (1|condition) + duration_min + age_days + end_heart_rate + end_temp
...
saveToPDF("../results/voom/voom.eBayes.finalmodel.pdf", width = 9, height = 5)
...
plotCorrMatrix( C ) 
saveToPDF("../results/variance/CCA_with_PCA.pdf", width = 4, height = 4)
...
supplementalFigure4
path <- paste0("../results/manuscript_figures/Supplemental_figure_2_bulkRNA_variance")
